  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        // Check if user has access to enterprise plan
        if (user.plan !== 'enterprise') {
          console.log(`User has ${user.plan} plan, cannot access enterprise dashboard`);
          setAuthorized(false);
          setLoading(false);
          return;
        }
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
            ...
            <p className="text-sm text-yellow-800">
              This dashboard is only available for Enterprise plan users.
            </p>
          </div>
          <button
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-yellow-100 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Welcome to Your Enterprise Dashboard!</h2>
          <p className="text-gray-600">
            Experience the full power of Eatnify with unlimited features.
          </p>
        </div>
      </div>
    </div>
