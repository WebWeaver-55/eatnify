  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        // Check if user has access to starter plan
        if (user.plan !== 'starter') {
          console.log(`User has ${user.plan} plan, cannot access starter dashboard`);
          setAuthorized(false);
          setLoading(false);
          return;
        }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-orange-100 p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            You don't have permission to access the Starter dashboard.
          </p>
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
            <AlertTriangle className="w-5 h-5 text-yellow-600 mx-auto mb-2" />
            <p className="text-sm text-yellow-800">
              This dashboard is only available for Starter plan users.
            </p>
          </div>
          <button
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Welcome to Your Starter Dashboard!</h2>
          <p className="text-gray-600">
            Start managing your restaurant with our Starter plan features.
          </p>
        </div>
      </div>
    </div>
